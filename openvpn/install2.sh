...
function installOpenVPN() {
	...
	mkdir -p /etc/openvpn /opt/openvpn/clients /opt/openvpn /opt/openvpn/google-auth /var/log/openvpn
	...
	if [[ ! -e /etc/openvpn/server.conf ]]; then
		if [[ $OS =~ (debian|ubuntu) ]]; then
			...
			dnf install -y iptables openssl wget ca-certificates curl policycoreutils-python-utils
			dnf install -y https://dl.fedoraproject.org/pub/epel/epel-release-latest-9.noarch.rpm
			dnf install -y cmake3 git lz4 lz4-devel lzo-devel google-authenticator qrencode pam-devel pwgen
...
${hostlist}
    ...
    """
    echo "${content}" | mailx -s "Your OpenVPN profile" -a "${homeDir}/${CLIENT}.ovpn" -a "/opt/openvpn/google-auth/${CLIENT}.png" -r "test<test@fake.com>" "${EMAIL}" || { echo "${R}${B}error mailing profile to client: ${CLIENT}${C}"; exit 1; }
}
...
function newClient() {
	...
	emailProfile "${CLIENT}" "${PW}" "rhodes.will@outlook.com" || { echo -e "${R}${B}Error sending profile to new user ${CLIENT} ${C}"; exit 1; }
